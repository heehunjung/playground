package heehunjun.playground.controller.tool.token.kakaoToken;
...
public class KakaoOAuthClient {
    ...
    public KakaoOAuthClient(final RestTemplateBuilder restTemplateBuilder) {
        this.kakaoProperties = new KakaoProperties();
        this.restTemplate = restTemplateBuilder.build();
    }

    /**
     * 구글 OAuth 는 Access Token 을 받아오면서 IdToken 도 받아와서 거기서 필요한 정보는 다 가져옴 (한번 더 Access Token으로 요청도 가능) 근데, Kakao 는 Access
     * Token 을 받고 한번 더 해당 토큰과 요청 해야 해 -> getUserInfo 추가됨 제네릭 사용해서 GoogleClient 와 추상화 ? ? ?
     */
